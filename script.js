...
document.querySelector('[data-enquiry-form]')?.addEventListener('submit',event=>{event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const subject=`Project enquiry from ${data.get('name')}`;const body=[`Name: ${data.get('name')}`,`Email: ${data.get('email')}`,`Scope: ${data.get('scope')}`,'',String(data.get('message'))].join('\n');window.location.href=`mailto:mackeygrip@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;const formStatus=form.querySelector('[data-form-status]');if(formStatus){formStatus.textContent='Your email app should open with the enquiry prepared. If it does not, email mackeygrip@gmail.com directly.';formStatus.classList.add('active')}});
